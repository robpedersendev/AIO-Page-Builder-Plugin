...
final class Rendering_Provider implements Service_Provider_Interface {
	...
	public function register( Service_Container $container ): void {
		$container->register(
			'section_render_context_builder',
			function (): Section_Render_Context_Builder {
				return new Section_Render_Context_Builder();
			}
		);

		$container->register(
			'smart_omission_service',
			function (): Smart_Omission_Service {
				return new Smart_Omission_Service();
			}
		);

		$container->register(
			'animation_fallback_service',
			function (): Animation_Fallback_Service {
				return new Animation_Fallback_Service();
			}
		);

		$container->register(
			'animation_tier_resolver',
			function (): Animation_Tier_Resolver {
				return new Animation_Tier_Resolver();
			}
		);

		$container->register(
			'section_renderer_base',
			function () use ( $container ): Section_Renderer_Base {
		...
		$container->register(
			'generateblocks_compatibility_layer',
			function (): GenerateBlocks_Compatibility_Layer {
				return new GenerateBlocks_Compatibility_Layer( GenerateBlocks_Compatibility_Layer::default_availability_check() );
			}
		);

		$container->register(
			'form_provider_registry',
			function (): Form_Provider_Registry {
				return new Form_Provider_Registry();
			}
		);

		$container->register(
			'form_provider_picker_discovery',
			function () use ( $container ): Form_Provider_Picker_Discovery_Service {
		...
		$container->register(
			'form_provider_picker_cache',
			function (): Form_Provider_Picker_Cache_Service {
				return new Form_Provider_Picker_Cache_Service();
			}
		);

		$container->register(
			'form_provider_availability_service',
			function () use ( $container ): Form_Provider_Availability_Service {
				$registry  = $container->get( 'form_provider_registry' );
		...
		$container->register(
			'form_provider_health_summary_service',
			function () use ( $container ): Form_Provider_Health_Summary_Service {
				$registry     = $container->get( 'form_provider_registry' );
		...
		$container->register(
			'native_block_assembly_pipeline',
			function () use ( $container ): Native_Block_Assembly_Pipeline {
		...
		$container->register(
			'page_instantiation_payload_builder',
			function (): Page_Instantiation_Payload_Builder {
		...
		$container->register(
			'page_instantiator',
			function () use ( $container ): Page_Instantiator {
		...
		$container->register(
			'content_survivability_checker',
			function (): Content_Survivability_Checker {
				return new Content_Survivability_Checker();
			}
		);

		$container->register(
			'rendering_diagnostics_service',
			function (): Rendering_Diagnostics_Service {
		...
		$container->register(
			...
			function (): Render_Preview_Helper {
				return new Render_Preview_Helper();
			}
		);

		$container->register(
			'render_asset_controller',
			function (): Render_Asset_Controller {
				return new Render_Asset_Controller();
			}
		);

		$container->register(
			'lpagery_token_compatibility_service',
			function (): LPagery_Token_Compatibility_Service {
		...
		$container->register(
			...
			function () use ( $container ): Library_LPagery_Compatibility_Service {

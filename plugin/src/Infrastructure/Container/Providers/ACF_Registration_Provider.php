...
final class ACF_Registration_Provider implements Service_Provider_Interface {
	...
	public function register( Service_Container $container ): void {
		$container->register(
			'acf_field_builder',
			function (): ACF_Field_Builder {
				return new ACF_Field_Builder();
			}
		);
		$container->register(
			'acf_group_builder',
			function () use ( $container ): ACF_Group_Builder {
				return new ACF_Group_Builder( $container->get( 'acf_field_builder' ) );
			}
		);
		$container->register(
			'acf_group_registrar',
			function () use ( $container ): ACF_Group_Registrar {
		...
		$container->register(
			'acf_registration_request_context',
			function (): Registration_Request_Context {
				return new Registration_Request_Context();
			}
		);
		$container->register(
			'acf_group_key_section_key_resolver',
			function (): Group_Key_Section_Key_Resolver {
				return new Group_Key_Section_Key_Resolver();
			}
		);
		$container->register(
			'page_section_key_cache_service',
			function (): Page_Section_Key_Cache_Service {
				return new Page_Section_Key_Cache_Service();
			}
		);
		$container->register(
			'acf_registration_diagnostics_service',
			function (): ACF_Registration_Diagnostics_Service {
				return new ACF_Registration_Diagnostics_Service();
			}
		);
		$container->register(
			'acf_registration_benchmark_service',
			function () use ( $container ): ACF_Registration_Benchmark_Service {
				return new ACF_Registration_Benchmark_Service( $container->get( 'acf_registration_diagnostics_service' ) );
			}
		);
		$container->register(
			'acf_existing_page_registration_context_resolver',
			function () use ( $container ): Existing_Page_ACF_Registration_Context_Resolver {
		...
		$container->register(
			'acf_new_page_registration_context_resolver',
			function () use ( $container ): New_Page_ACF_Registration_Context_Resolver {
		...
		$container->register(
			'admin_post_edit_context_resolver',
			function (): Admin_Post_Edit_Context_Resolver {
				return new Admin_Post_Edit_Context_Resolver();
			}
		);
		$container->register(
			'scoped_registration_inspection_service',
			function () use ( $container ): Scoped_Registration_Inspection_Service {
		...
		$container->register(
			'acf_registration_bootstrap_controller',
			function () use ( $container ): ACF_Registration_Bootstrap_Controller {

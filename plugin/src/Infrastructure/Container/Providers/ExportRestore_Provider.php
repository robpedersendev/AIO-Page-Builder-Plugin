...
final class ExportRestore_Provider implements Service_Provider_Interface {
	...
	public function register( Service_Container $container ): void {
		$container->register(
			'import_validator',
			function () use ( $container ): \AIOPageBuilder\Domain\ExportRestore\Import\Import_Validator {
		...
		$container->register(
			...
			function (): \AIOPageBuilder\Domain\ExportRestore\Import\Conflict_Resolution_Service {
				return new \AIOPageBuilder\Domain\ExportRestore\Import\Conflict_Resolution_Service();
			}
		);
		$container->register(
			'template_library_export_validator',
			function () use ( $container ): Template_Library_Export_Validator {
		...
		$container->register(
			'template_library_restore_validator',
			function () use ( $container ): Template_Library_Restore_Validator {
				$section_appendix = $container->has( 'section_inventory_appendix_generator' ) ? $container->get( 'section_inventory_appendix_generator' ) : null;
				$page_appendix    = $container->has( 'page_template_inventory_appendix_generator' ) ? $container->get( 'page_template_inventory_appendix_generator' ) : null;
		...
		$container->register(
			'restore_pipeline',
			function () use ( $container ): \AIOPageBuilder\Domain\ExportRestore\Import\Restore_Pipeline {
				global $wpdb;
		...
		$container->register(
			'export_manifest_builder',
			function (): Export_Manifest_Builder {
				return new Export_Manifest_Builder();
			}
		);
		$container->register(
			'export_zip_packager',
			function () use ( $container ): Export_Zip_Packager {
				return new Export_Zip_Packager( $container->get( 'plugin_path_manager' ) );
			}
		);
		$container->register(
			'export_token_set_reader',
			function (): Export_Token_Set_Reader {
				global $wpdb;
		...
		$container->register(
			'template_library_support_summary_builder',
			function () use ( $container ): Template_Library_Support_Summary_Builder {
		...
		$container->register(
			...
			function () use ( $container ): Support_Package_Generator {
		...
		$container->register(
			...
			function (): Uninstall_Cleanup_Service {
				return new Uninstall_Cleanup_Service();
			}
		);
		$container->register(
			'uninstall_export_prompt_service',
			function () use ( $container ): Uninstall_Export_Prompt_Service {
		...
		$container->register(
			...
			function () use ( $container ): Template_Library_Lifecycle_Summary_Builder {
		...
		$container->register(
			'import_export_state_builder',
			function () use ( $container ): Import_Export_State_Builder {

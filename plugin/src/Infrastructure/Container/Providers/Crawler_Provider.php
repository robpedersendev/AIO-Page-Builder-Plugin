...
final class Crawler_Provider implements Service_Provider_Interface {
	...
	public function register( Service_Container $container ): void {
		$container->register(
			'crawl_snapshot_repository',
			function (): Crawl_Snapshot_Repository {
				global $wpdb;
				return new Crawl_Snapshot_Repository( $wpdb );
			}
		);
		$container->register(
			'crawl_profile_service',
			function (): Crawl_Profile_Service {
				return new Crawl_Profile_Service();
			}
		);
		$container->register(
			'crawl_template_family_matcher',
			function (): Crawl_Template_Family_Matcher {
				return new Crawl_Template_Family_Matcher();
			}
		);
		$container->register(
			'crawl_snapshot_service',
			function () use ( $container ): Crawl_Snapshot_Service {
		...
		$container->register(
			'url_discovery_service',
			function () use ( $container ): URL_Discovery_Service {
				return new URL_Discovery_Service( $container->get( 'url_normalizer' ) );
			}
		);
		$container->register(
			'fetch_request_policy',
			function (): Fetch_Request_Policy {
				return new Fetch_Request_Policy();
			}
		);
		$container->register(
			'html_fetcher',
			function () use ( $container ): HTML_Fetcher {
		...
		$container->register(
			'meaningful_page_classifier',
			function () use ( $container ): Meaningful_Page_Classifier {
				return new Meaningful_Page_Classifier( $container->get( 'duplicate_detector' ) );
			}
		);
		$container->register(
			'navigation_extractor',
			function (): Navigation_Extractor {
		...
		$container->register(
			'recrawl_comparison_service',
			function () use ( $container ): Recrawl_Comparison_Service {

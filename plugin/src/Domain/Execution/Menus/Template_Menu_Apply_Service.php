...
final class Template_Menu_Apply_Service implements Template_Menu_Apply_Service_Interface {
	...
	public function apply( array $envelope ): Template_Menu_Apply_Result {
		...
		$hierarchy_summary = array(
			'items_ordered_by_class' => array_map(
				function ( array $item ): array {
					$out = array( 'title' => $item['title'] ?? '', 'object_id' => $item['object_id'] ?? 0 );
		...
		if ( $action === 'update_existing' ) {
			...
			if ( $menu_id <= 0 ) {
				return Template_Menu_Apply_Result::failure(
					...
					array_merge( $validation, array( 'valid' => true, 'resolved_menu_id' => 0 ) ),
					$hierarchy_summary,
					array()
				);
			}
			...
			$create_result = $this->menu_job_service->run( array_merge( $envelope, array(
				Execution_Action_Contract::ENVELOPE_TARGET_REFERENCE => array_merge( $target, array(
		...
		return Template_Menu_Apply_Result::success(
			$menu_id,
			array_merge( $validation, array( 'valid' => true, 'resolved_menu_id' => $menu_id ) ),
			$hierarchy_summary,
			$per_item_status,
			$artifacts
		);
	...
	private function apply_items_with_hierarchy( int $menu_id, array $ordered_items, array &$per_item_status ): int {
		...
		foreach ( $ordered_items as $item ) {
			...
			if ( $title === '' && $url === '' && $object_id <= 0 ) {
				$per_item_status[] = array( 'status' => 'skipped', 'reason' => 'missing_title_url_object_id' );
				continue;
			}

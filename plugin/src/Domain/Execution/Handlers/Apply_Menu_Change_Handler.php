<?php
/**
 * Execution handler for update_menu actions (spec §34, §40.1, §40.2; Prompt 083).
 *
 * Delegates to Menu_Change_Job_Service. Governed menu create/rename/replace/update_existing
 * and location assignment only.
 *
...
final class Apply_Menu_Change_Handler implements Execution_Handler_Interface {
	...
	public function __construct( Menu_Change_Job_Service_Interface $job_service ) {
		$this->job_service = $job_service;
	}

	/**
	 * Executes menu change. Envelope validated by Single_Action_Executor.
	 *
	 * @param array<string, mixed> $envelope Governed action envelope.
	 * @return array<string, mixed> success, message, artifacts (menu_id, location_assigned, etc.).
	 */

...
final class Tokens_Step_UI_Service {
	...
	private function build_detail_panel( array $items, ?string $selected_item_id, array $capabilities ): array {
		...
		foreach ( $items as $item ) {
			...
			$detail_panel['sections'] = array(
				array( 'heading' => \__( 'Token', 'aio-page-builder' ), 'key' => 'token', 'content_lines' => array(
	...
	private function build_token_set_summary( array $items ): array {
		...
		return array( 'groups' => $groups, 'total' => count( $items ) );
	}
	...
	private function placeholder_bulk_states( int $eligible_count, array $capabilities, array $selected_item_ids, array $rows ): array {
		...
		return array(
			Bulk_Action_Bar_Component::CONTROL_APPLY_TO_ALL     => array( 'enabled' => false, 'label' => \__( 'Apply all tokens', 'aio-page-builder' ), 'count_eligible' => $eligible_count ),
	...
	private function step_messages( int $total, int $eligible ): array {
		if ( $total === 0 ) {
			return array( array( 'severity' => 'info', 'message' => \__( 'No design token recommendations for this plan.', 'aio-page-builder' ), 'level' => 'step' ) );
		}
		if ( $eligible === 0 ) {
			return array( array( 'severity' => 'success', 'message' => \__( 'All token recommendations have been reviewed.', 'aio-page-builder' ), 'level' => 'step' ) );
		}
		return array( array( 'severity' => 'info', 'message' => sprintf( \_n( '%d token pending review.', '%d tokens pending review.', $eligible, 'aio-page-builder' ), $eligible ), 'level' => 'step' ) );
	}
	...
	private function empty_workspace(): array {
		return array(
			...
			'bulk_action_states'     => array(
				Bulk_Action_Bar_Component::CONTROL_APPLY_TO_ALL => array( 'enabled' => false, 'label' => \__( 'Apply all tokens', 'aio-page-builder' ), 'count_eligible' => 0 ),
			...
			'step_messages'         => array(),
			'token_set_summary'      => array( 'groups' => array(), 'total' => 0 ),
			'token_diff_placeholder' => array( 'current_value' => '', 'proposed_value' => '', 'applied' => false ),
		);
	}
}

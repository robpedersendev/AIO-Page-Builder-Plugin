<?php
/**
 * Step 1 bulk and single approve/deny state handling (spec §32.5–32.8).
 *
 * Applies individual or bulk approve/deny to existing-page items; persists via repository.
 * Does not execute page mutations. Mutual exclusivity: after bulk approve there are no
 * pending items left so bulk deny is ineligible; after bulk deny same for bulk approve.
 *
...
/**
 * Single and bulk approve/deny for Step 1 (existing_page_changes). Caller must verify capability and nonce.
 */
final class Existing_Page_Update_Bulk_Action_Service {
	...
	/**
	 * Step index for existing page changes in canonical step order (Build_Plan_Schema).
	 */
	public const STEP_INDEX_EXISTING_PAGE_CHANGES = 1;
	...
	/**
	 * Bulk approve all pending items in Step 1 (spec §32.6). Requires confirmation from UI.
	 *
	 * @param int $plan_post_id Plan post ID.
	...
	public function bulk_approve_all_eligible( int $plan_post_id ): int {
		return $this->repository->update_plan_step_items_by_status(
			$plan_post_id,
			self::STEP_INDEX_EXISTING_PAGE_CHANGES,
	...
	/**
	 * Bulk deny all pending items in Step 1 (spec §32.7). Preserves state in plan record.
	 *
	 * @param int $plan_post_id Plan post ID.
	 * @return int Number of items updated.
	 */
	...
	private function set_item_status_if_pending( int $plan_post_id, string $item_id, string $new_status ): bool {
		...
		foreach ( $items as $i => $item ) {
			...
			if ( (string) ( $item['item_id'] ?? '' ) === $item_id && (string) ( $item['status'] ?? '' ) === Build_Plan_Item_Statuses::PENDING ) {
				...
				$definition[ Build_Plan_Schema::KEY_STEPS ][ self::STEP_INDEX_EXISTING_PAGE_CHANGES ][ Build_Plan_Item_Schema::KEY_ITEMS ] = $items;

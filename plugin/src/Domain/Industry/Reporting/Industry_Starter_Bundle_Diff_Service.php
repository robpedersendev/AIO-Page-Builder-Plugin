...
final class Industry_Starter_Bundle_Diff_Service {
	...
	private function build_diff_rows( array $bundles ): array {
		...
		$fields = array(
			'page_families'        => array(
				'label' => __( 'Page family emphasis', 'aio-page-builder' ),
				'type'  => 'array',
			),
			'page_template_refs'   => array(
				'label' => __( 'Page template refs', 'aio-page-builder' ),
				'type'  => 'array',
			),
			'section_refs'         => array(
				'label' => __( 'Section refs', 'aio-page-builder' ),
				'type'  => 'array',
			),
			'cta_guidance_ref'     => array(
				'label' => __( 'CTA guidance ref', 'aio-page-builder' ),
				'type'  => 'string',
			),
			'lpagery_guidance_ref' => array(
				'label' => __( 'LPagery guidance ref', 'aio-page-builder' ),
				'type'  => 'string',
			),
			'token_preset_ref'     => array(
				'label' => __( 'Token preset ref', 'aio-page-builder' ),
				'type'  => 'string',
			),
			'metadata_notes'       => array(
				'label' => __( 'Preset / metadata notes', 'aio-page-builder' ),
				'type'  => 'string',
			),
		);
		...
		foreach ( $fields as $field => $config ) {
			...
			for ( $i = 1; $i < $n; $i++ ) {
				...
				} elseif ( (string) $first !== (string) $values[ $i ] ) {
						$changed = true;
						break;
				}
			}

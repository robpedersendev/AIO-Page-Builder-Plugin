...
final class Demo_Fixture_Generator {
		...
		return array(
			Profile_Schema::ROOT_BRAND => array(
				...
				Profile_Schema::BRAND_ASSET_REFERENCES => array(
					array( 'role' => 'logo', 'ref' => 'demo_asset_logo_001', 'notes' => 'Synthetic reference.' ),
				),
			),
			Profile_Schema::ROOT_BUSINESS => array(
				Profile_Schema::BUSINESS_PERSONAS => array(
					array( 'name' => 'Demo Persona', 'description' => 'Synthetic persona for fixture.', 'priority' => 1 ),
				),
				Profile_Schema::BUSINESS_SERVICES_OFFERS => array(
					array( 'name' => 'Demo Service', 'description' => 'Synthetic service.', 'dedicated_pages_likely' => 'yes' ),
				),
				Profile_Schema::BUSINESS_COMPETITORS => array(),
				Profile_Schema::BUSINESS_GEOGRAPHY => array( 'in_person_vs_remote' => 'both', 'notes' => 'Demo geography.' ),
			),
		);
	}
	...
	public function get_ai_run_fixture(): array {
		...
		foreach ( array( Artifact_Category_Keys::NORMALIZED_OUTPUT, Artifact_Category_Keys::USAGE_METADATA ) as $cat ) {
			$artifacts[ $cat ] = array( 'placeholder' => true, 'category' => $cat, self::SYNTHETIC_MARKER => true );
		}
		return array(
			...
			array(
				...
				Build_Plan_Item_Schema::KEY_ITEMS   => array(
					array(
						...
						Build_Plan_Item_Schema::KEY_PAYLOAD   => array( 'target' => 'demo_page', 'action' => 'update_content' ),

<?php
/**
 * Seeds the feature/benefit/value library batch (SEC-03) into the section template registry (spec §12, Prompt 149).
...
/**
 * One-way seed: writes feature/benefit/value batch section definitions to the section repository.
 */
final class Feature_Benefit_Value_Library_Batch_Seeder {
	...
	public static function run( Section_Template_Repository $section_repo ): array {
		$errors      = array();
		$section_ids = array();
		foreach ( Feature_Benefit_Value_Library_Batch_Definitions::all_definitions() as $definition ) {
			$id = $section_repo->save_definition( $definition );
			if ( $id <= 0 ) {
				$key = (string) ( $definition[ Section_Schema::FIELD_INTERNAL_KEY ] ?? 'unknown' );
				$errors[] = sprintf( __( 'Failed to save feature/benefit section: %s', 'aio-page-builder' ), $key );
				continue;
			}
			$section_ids[] = $id;
		}

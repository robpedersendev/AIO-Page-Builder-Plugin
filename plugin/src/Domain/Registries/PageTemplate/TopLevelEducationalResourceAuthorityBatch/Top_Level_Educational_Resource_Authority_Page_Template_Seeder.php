<?php
/**
 * Seeds the top-level educational/resource/authority page template batch (spec §13, §14.3, §16, Prompt 163).
 * Idempotent: re-running overwrites existing definitions for the same keys.
 * Run after section library (all section batches + expansion pack) so section keys exist.
 *
...
/**
 * One-way seed: writes top-level educational/resource/authority page templates to the page template repository.
 */
final class Top_Level_Educational_Resource_Authority_Page_Template_Seeder {
	...
	public static function run( Page_Template_Repository $page_repo ): array {
		...
		foreach ( Top_Level_Educational_Resource_Authority_Page_Template_Definitions::all_definitions() as $definition ) {

<?php
/**
 * Enriches section directory state with industry filter and badges (industry-admin-screen-contract, industry-section-recommendation-contract).
...
namespace AIOPageBuilder\Admin\Screens\Sections;
...
/**
 * Adds industry_view filter and industry_badges_by_key to section directory state. Safe fallback when profile missing.
 */
final class Industry_Section_Library_Filter_Controller {

	/** @var Industry_Section_Library_Read_Model_Builder */
	...
	public function __construct(
		Industry_Section_Library_Read_Model_Builder $read_model_builder,
	 ...
	 *
	 * @param array<string, mixed> $state          State from Section_Template_Directory_State_Builder (with list_result.rows when view is list/search).
	...
	public function enrich_state( array $state, array $request_params ): array {
		...
		foreach ( $rows as $row ) {
			...
			if ( $key === '' ) {
				$key = isset( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) && is_string( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) ? trim( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) : '';
			}
			if ( $key !== '' ) {
				$sections[] = $row;
			}
		...
		$item_views = $this->read_model_builder->build( $profile, $primary_pack, $sections, $industry_view );
		$badges_by_key = array();
		$ordered_keys = array();
		...
		if ( $industry_view !== Industry_Section_Library_Read_Model_Builder::VIEW_FULL_LIBRARY && count( $ordered_keys ) > 0 ) {
			$rows_by_key = array();
			foreach ( $rows as $row ) {
				$key = isset( $row['internal_key'] ) && is_string( $row['internal_key'] ) ? trim( $row['internal_key'] ) : ( isset( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) && is_string( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) ? trim( $row[ Section_Schema::FIELD_INTERNAL_KEY ] ) : '' );
				if ( $key !== '' ) {
					$rows_by_key[ $key ] = $row;
				}
			}

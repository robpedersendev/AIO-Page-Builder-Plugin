...
final class Template_Menu_Apply_Result_Test extends TestCase {
	...
	public static function example_menu_apply_execution_result_payload(): array {
		$validation = array( 'valid' => true, 'location_slug' => 'primary', 'missing_location' => false, 'resolved_menu_id' => 10 );
		$hierarchy = array(
			'items_ordered_by_class' => array(
				array( 'title' => 'Home', 'object_id' => 1, 'page_class' => 'top_level' ),
				array( 'title' => 'Services', 'object_id' => 2, 'page_class' => 'hub' ),
				array( 'title' => 'Consulting', 'object_id' => 3, 'page_class' => 'child_detail', 'parent_page_id' => 2 ),
			),
			'applied_count' => 3,
			'warnings'      => array(),
		);
		$per_item = array(
			array( 'status' => 'applied', 'title' => 'Home', 'object_id' => 1, 'menu_item_id' => 101 ),
			array( 'status' => 'applied', 'title' => 'Services', 'object_id' => 2, 'menu_item_id' => 102 ),
			array( 'status' => 'applied', 'title' => 'Consulting', 'object_id' => 3, 'menu_item_id' => 103 ),
		);
		return array(
			'success'                      => true,
	...
	public function test_success_factory_and_to_menu_apply_execution_result(): void {
		$validation = array( 'valid' => true, 'location_slug' => 'primary' );
		$hierarchy = array( 'items_ordered_by_class' => array(), 'applied_count' => 2, 'warnings' => array() );
		$per_item = array(
			array( 'status' => 'applied', 'title' => 'A', 'object_id' => 1 ),
			array( 'status' => 'applied', 'title' => 'B', 'object_id' => 2 ),
		);
	...
	public function test_failure_factory_includes_validation_and_message(): void {
		...
		$result = Template_Menu_Apply_Result::failure(
			...
			$validation,
			array( 'items_ordered_by_class' => array(), 'applied_count' => 0, 'warnings' => array() ),
			array()
		);
	...
	public function test_to_handler_result_includes_artifacts_for_snapshot(): void {
		$validation = array( 'valid' => true, 'location_slug' => 'primary' );
		$hierarchy = array( 'applied_count' => 1, 'items_ordered_by_class' => array(), 'warnings' => array() );

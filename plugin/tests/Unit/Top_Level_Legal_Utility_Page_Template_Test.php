<?php
/**
 * Unit tests for top-level legal/utility page template batch (Prompt 156): schema validity,
 * CTA sequencing, bottom-CTA, no-adjacent-CTA, section-count targets, one-pager, preview integrity,
 * semantic/accessibility compliance (one-pager flow), export (spec §13, §15.10, §51).
 *
...
use AIOPageBuilder\Domain\Registries\PageTemplate\TopLevelLegalUtilityBatch\Top_Level_Legal_Utility_Page_Template_Definitions;
...
require_once $plugin_root . '/src/Domain/Registries/PageTemplate/TopLevelLegalUtilityBatch/Top_Level_Legal_Utility_Page_Template_Definitions.php';
...
final class Top_Level_Legal_Utility_Page_Template_Test extends TestCase {
	...
	/** Allowed template_family values for PT-02 (Prompt 156). */
	private const ALLOWED_LEGAL_UTILITY_FAMILIES = array( 'privacy', 'terms', 'accessibility', 'support', 'disclosure', 'utility' );

	public function test_batch_has_expected_number_of_templates(): void {
		$defs = Top_Level_Legal_Utility_Page_Template_Definitions::all_definitions();
		$keys = Top_Level_Legal_Utility_Page_Template_Definitions::template_keys();
	...
	public function test_each_definition_has_required_schema_fields(): void {
		...
		foreach ( Top_Level_Legal_Utility_Page_Template_Definitions::all_definitions() as $def ) {
	...
	public function test_each_definition_has_template_category_class_and_legal_utility_family(): void {
		foreach ( Top_Level_Legal_Utility_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertContains( $family, self::ALLOWED_LEGAL_UTILITY_FAMILIES, 'template_family must be one of privacy, terms, accessibility, support, disclosure, utility' );
		}
	}
	...
	public function test_preview_metadata_synthetic_only(): void {
		foreach ( Top_Level_Legal_Utility_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertTrue( $def['preview_metadata']['synthetic'], 'Legal/utility templates must use synthetic preview data only' );
		}
	}

	public function test_one_pager_includes_flow_or_accessibility_notes(): void {
		foreach ( Top_Level_Legal_Utility_Page_Template_Definitions::all_definitions() as $def ) {
			$one_pager = $def[ Page_Template_Schema::FIELD_ONE_PAGER ] ?? array();
			$this->assertIsArray( $one_pager );
			$has_flow = isset( $one_pager['page_flow_explanation'] ) && (string) $one_pager['page_flow_explanation'] !== '';
			$this->assertTrue( $has_flow, 'One-pager should include page_flow_explanation for semantic/accessibility clarity' );
		}
	}
	...
	public function test_batch_id_constant(): void {
		$this->assertSame( 'PT-02', Top_Level_Legal_Utility_Page_Template_Definitions::BATCH_ID );
	}
}

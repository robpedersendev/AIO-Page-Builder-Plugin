...
final class Large_Composition_Validator_Test extends TestCase {
	...
	public function test_bottom_cta_missing_adds_blocker(): void {
		$this->seed_sections( array(
			'hero_a' => array( 'cta_classification' => 'none', 'preview_description' => 'Hero' ),
			'cta_a'  => array( 'cta_classification' => 'primary_cta', 'preview_description' => 'CTA' ),
		) );
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta_a', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'hero_a', Composition_Schema::SECTION_ITEM_POSITION => 1 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_adjacent_cta_adds_blocker(): void {
		$this->seed_sections( array(
			'cta_one' => array( 'cta_classification' => 'primary_cta', 'preview_description' => 'CTA 1' ),
			'cta_two' => array( 'cta_classification' => 'contact_cta', 'preview_description' => 'CTA 2' ),
		) );
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta_one', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta_two', Composition_Schema::SECTION_ITEM_POSITION => 1 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_cta_count_below_minimum_when_class_set(): void {
		$section_defs = array();
		for ( $i = 0; $i < 5; $i++ ) {
			$section_defs[ 'sec_' . $i ] = array( 'cta_classification' => $i < 2 ? 'primary_cta' : 'none', 'preview_description' => 'S' );
		}
		$this->seed_sections( $section_defs );
		$composition = array(
			'template_category_class' => 'hub',
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'sec_0', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'sec_1', Composition_Schema::SECTION_ITEM_POSITION => 1 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'sec_2', Composition_Schema::SECTION_ITEM_POSITION => 2 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'sec_3', Composition_Schema::SECTION_ITEM_POSITION => 3 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'sec_4', Composition_Schema::SECTION_ITEM_POSITION => 4 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_non_cta_count_below_minimum_when_class_set(): void {
		...
		foreach ( array_keys( $section_defs ) as $pos => $key ) {
			$ordered[] = array( Composition_Schema::SECTION_ITEM_KEY => $key, Composition_Schema::SECTION_ITEM_POSITION => $pos );
		}
		$composition = array(
			'template_category_class' => 'hub',
	...
	public function test_legacy_compatibility_adjacency_mapped_to_compatibility_violations(): void {
		$this->seed_sections( array(
			'a' => array( 'cta_classification' => 'none', 'preview_description' => 'A' ),
			'b' => array( 'cta_classification' => 'primary_cta', 'preview_description' => 'B' ),
		) );
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'a', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'b', Composition_Schema::SECTION_ITEM_POSITION => 1 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_section_missing_preview_adds_preview_readiness_warning(): void {
		...
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'no_preview', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_one_pager_missing_adds_preview_readiness_warning(): void {
		$this->seed_sections( array(
			'cta' => array( 'cta_classification' => 'primary_cta', 'preview_description' => 'x' ),
		) );
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
	...
	public function test_example_composition_validation_result_payload_with_blockers_and_warnings(): void {
		$this->seed_sections( array(
			'cta1' => array( 'cta_classification' => 'primary_cta', 'preview_description' => 'CTA1' ),
			'cta2' => array( 'cta_classification' => 'contact_cta', 'preview_description' => 'CTA2' ),
			'hero' => array( 'cta_classification' => 'none' ),
		) );
		$composition = array(
			Composition_Schema::FIELD_ORDERED_SECTION_LIST => array(
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta1', Composition_Schema::SECTION_ITEM_POSITION => 0 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'cta2', Composition_Schema::SECTION_ITEM_POSITION => 1 ),
				array( Composition_Schema::SECTION_ITEM_KEY => 'hero', Composition_Schema::SECTION_ITEM_POSITION => 2 ),
			),
			Composition_Schema::FIELD_SOURCE_TEMPLATE_REF  => '',
			Composition_Schema::FIELD_HELPER_ONE_PAGER_REF => '',
		);
		...
		$example = array(
			'valid'                      => false,
			'blockers'                   => array(
				array( 'code' => 'adjacent_cta_violation', 'message' => 'Two CTA sections are adjacent.' ),
				array( 'code' => 'bottom_cta_missing', 'message' => 'Last section must be CTA-classified.' ),
			),
			'warnings'                   => array(),
			'cta_rule_violations'        => array(
				array( 'code' => 'adjacent_cta_violation', 'message' => 'Two CTA sections are adjacent; add a non-CTA section between them.', 'position' => 0 ),
				array( 'code' => 'bottom_cta_missing', 'message' => 'Last section must be CTA-classified.', 'position' => 2 ),
			),
			'compatibility_violations'   => array(),
			'preview_readiness_warnings' => array(
				array( 'code' => 'section_missing_preview', 'message' => 'Section hero has no preview data.' ),
				array( 'code' => 'one_pager_missing', 'message' => 'Composition has no one-pager reference.' ),
			),
			'legacy_codes'               => array(),
		);

...
final class Diagnostics_Wpdb_Stub {
	...
	public function prepare( string $query, ...$args ): string {
		$i = 0;
		return preg_replace_callback( '/%[sd]/', function () use ( $args, &$i ) {
			$v = $args[ $i++ ] ?? null;
	...
	public function query( string $sql ): int|false {
		...
		if ( strpos( $sql, 'INSERT INTO' ) !== false ) {
			...
			$store[] = array( 'id' => $this->insert_id, 'map_type' => $map_type, 'source_ref' => $source_ref, 'target_ref' => $target_ref );
			return 1;
		}
		return 1;
	}
	...
	public function get_results( string $query, $output = OBJECT ): array {
		...
		if ( ! is_array( $store ) ) { return array(); }
		$map_type = $source_ref = null;
		if ( preg_match( '/map_type\s*=\s*[\'"]([^\'"]*)[\'"]/', $query, $m ) ) { $map_type = $m[1]; }
		if ( preg_match( '/source_ref\s*=\s*[\'"]([^\'"]*)[\'"]/', $query, $m ) ) { $source_ref = $m[1]; }
		$out = array();
		...
		if ( preg_match( '/LIMIT\s+(\d+)/', $query, $m ) ) { $out = array_slice( $out, 0, (int) $m[1] ); }
		return $out;
	}
	...
	public function delete( string $table, array $where, $where_format = null ): int|false {
		...
		if ( ! is_array( $store ) ) { $store = array(); return 0; }
...
final class ACF_Diagnostics_Service_Test extends TestCase {
	...
	private function seed_section( string $key, array $definition ): void {
		$id = self::$seed_id++;
		$post = new \WP_Post( array( 'ID' => $id, 'post_type' => Object_Type_Keys::SECTION_TEMPLATE, 'post_title' => 'Test', 'post_status' => 'publish', 'post_name' => $key ) );
		$GLOBALS['_aio_wp_query_posts'][] = $post;
		$def = array_merge( array( 'internal_key' => $key, 'status' => 'active' ), $definition );
		$GLOBALS['_aio_post_meta'][ (string) $id ] = array( '_aio_internal_key' => $key, '_aio_status' => $def['status'] ?? 'active', '_aio_section_definition' => wp_json_encode( $def ) );
	}

	private function seed_template( string $key, array $definition ): void {
		$id = self::$seed_id++;
		$post = new \WP_Post( array( 'ID' => $id, 'post_type' => Object_Type_Keys::PAGE_TEMPLATE, 'post_title' => 'Test', 'post_status' => 'publish', 'post_name' => $key ) );
		$GLOBALS['_aio_wp_query_posts'][] = $post;
		$def = array_merge( array( 'internal_key' => $key ), $definition );
		$GLOBALS['_aio_post_meta'][ (string) $id ] = array( '_aio_internal_key' => $key, '_aio_status' => 'active', '_aio_page_template_definition' => wp_json_encode( $def ) );
	}
	...
	public function test_blueprint_summary_counts_valid_blueprints(): void {
		$this->seed_section( 'st_diag_hero', array(
			'field_blueprint' => array(
				...
				'section_version' => '1',
				'label'           => 'Diag Hero',
				'fields'          => array( array( 'name' => 'headline', 'type' => 'text', 'label' => 'Headline' ) ),
			),
		) );
		$this->seed_section( 'st_diag_faq', array(
			'field_blueprint' => array(
				'blueprint_id'    => 'acf_st_diag_faq',
				'section_key'     => 'st_diag_faq',
				'section_version' => '1',
				'label'           => 'Diag FAQ',
				'fields'          => array( array( 'name' => 'title', 'type' => 'text', 'label' => 'Title' ) ),
			),
		) );
	...
	public function test_registered_groups_summary_when_acf_unavailable(): void {
		$this->seed_section( 'st_diag_one', array(
			'field_blueprint' => array(
				'blueprint_id'    => 'acf_st_diag_one',
				'section_key'     => 'st_diag_one',
				'section_version' => '1',
				'label'           => 'One',
				'fields'          => array( array( 'name' => 'f1', 'type' => 'text', 'label' => 'F1' ) ),
			),
		) );
	...
	public function test_stale_items_aggregates_from_cleanup_advisor(): void {
		$this->seed_template( 'pt_diag_landing', array(
		...
		$this->diagnostics->get_page_assignments_summary();
		$store = &$GLOBALS['_aio_assign_store'];
		$store[] = array( 'id' => 1, 'map_type' => 'page_template', 'source_ref' => (string) $page_id, 'target_ref' => 'pt_diag_landing' );
		$store[] = array( 'id' => 2, 'map_type' => 'page_field_group', 'source_ref' => (string) $page_id, 'target_ref' => 'group_aio_st_diag_old' );
	...
	public function test_page_assignments_summary_counts_rows_and_pages(): void {
		$store = &$GLOBALS['_aio_assign_store'];
		$store[] = array( 'id' => 10, 'map_type' => 'page_field_group', 'source_ref' => '1001', 'target_ref' => 'group_aio_st01' );
		$store[] = array( 'id' => 11, 'map_type' => 'page_field_group', 'source_ref' => '1001', 'target_ref' => 'group_aio_st05' );
		$store[] = array( 'id' => 12, 'map_type' => 'page_field_group', 'source_ref' => '1002', 'target_ref' => 'group_aio_st01' );
		$store[] = array( 'id' => 13, 'map_type' => 'page_template', 'source_ref' => '1001', 'target_ref' => 'pt_landing' );

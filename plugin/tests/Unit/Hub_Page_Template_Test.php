<?php
/**
 * Unit tests for hub page template batch (Prompt 157): schema validity, hub-category tagging,
 * CTA counts (≥4), bottom-CTA, no-adjacent-CTA, section-count targets, one-pager, preview, export (spec §13, §14, §16).
 *
...
use AIOPageBuilder\Domain\Registries\PageTemplate\HubBatch\Hub_Page_Template_Definitions;
...
require_once $plugin_root . '/src/Domain/Registries/PageTemplate/HubBatch/Hub_Page_Template_Definitions.php';
...
final class Hub_Page_Template_Test extends TestCase {
	...
	/** Allowed template_family values for hub batch (Prompt 157). */
	private const ALLOWED_HUB_FAMILIES = array( 'services', 'products', 'offerings', 'directories', 'locations' );

	public function test_batch_has_expected_number_of_templates(): void {
		$defs = Hub_Page_Template_Definitions::all_definitions();
		$keys = Hub_Page_Template_Definitions::template_keys();
	...
	public function test_each_definition_has_hub_category_class_and_family(): void {
		foreach ( Hub_Page_Template_Definitions::all_definitions() as $def ) {
	...
	public function test_preview_metadata_synthetic(): void {
		foreach ( Hub_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertTrue( $def['preview_metadata']['synthetic'] );
		}
	}
	...
	public function test_batch_id_constant(): void {
		$this->assertSame( 'PT-03', Hub_Page_Template_Definitions::BATCH_ID );
	}
}

...
final class Section_Field_Blueprint_Test extends TestCase {
		...
		return array(
			...
			'fields'          => array(
				array(
					...
					'validation' => array( 'required' => true, 'maxlength' => 120 ),
					'lpagery' => array( 'token_compatible' => true, 'token_name' => '{{headline}}' ),
				),
				array(
		...
		return array(
			...
			'fields'          => array(
				...
				array(
					...
					'sub_fields'  => array(
						array( 'key' => 'field_st05_faq_items_question', 'name' => 'question', 'label' => 'Question', 'type' => 'text', 'required' => true ),
						array( 'key' => 'field_st05_faq_items_answer', 'name' => 'answer', 'label' => 'Answer', 'type' => 'wysiwyg', 'required' => true ),
					),
				),
			),
		);
	...
	public function test_validator_rejects_invalid_structure(): void {
		$validator = new Section_Field_Blueprint_Validator();
		$bad = array( 'blueprint_id' => 'bad', 'section_key' => 'x' );
	...
	public function test_validator_rejects_unsupported_field_type(): void {
		$validator = new Section_Field_Blueprint_Validator();
		$bp = $this->valid_minimal_blueprint();
		$bp['fields'][] = array( 'key' => 'field_st01_x', 'name' => 'x', 'label' => 'X', 'type' => 'flexible_content' );
	...
	public function test_validator_rejects_duplicate_field_keys(): void {
		$validator = new Section_Field_Blueprint_Validator();
		$bp = $this->valid_minimal_blueprint();
		$bp['fields'][] = array( 'key' => 'field_st01_hero_headline', 'name' => 'dup', 'label' => 'Dup', 'type' => 'text' );
		$errors = $validator->validate( $bp );
		$this->assertNotEmpty( $errors );

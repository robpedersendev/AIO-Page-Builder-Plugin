<?php
/**
 * Unit tests for child/detail variant expansion super-batch (Prompt 166): schema validity, child_detail tagging,
 * CTA compliance (≥5, last CTA, no adjacent), hierarchy metadata, preview, one-pager, differentiation notes, export (spec §13, §14.3, §16).
 *
...
use AIOPageBuilder\Domain\Registries\PageTemplate\ChildDetailVariantExpansionBatch\Child_Detail_Variant_Expansion_Page_Template_Definitions;
...
require_once $plugin_root . '/src/Domain/Registries/PageTemplate/ChildDetailVariantExpansionBatch/Child_Detail_Variant_Expansion_Page_Template_Definitions.php';
...
final class Child_Detail_Variant_Expansion_Page_Template_Test extends TestCase {
	...
	public function test_batch_has_large_variant_count(): void {
		$defs = Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions();
		$keys = Child_Detail_Variant_Expansion_Page_Template_Definitions::template_keys();
		$this->assertCount( count( $keys ), $defs );
		$this->assertGreaterThanOrEqual( 25, count( $defs ), 'Child/detail variant expansion batch must add a large volume of variants (≥25)' );
	}

	public function test_each_definition_has_required_schema_fields(): void {
		$required = Page_Template_Schema::get_required_fields();
		foreach ( Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions() as $def ) {
	...
	public function test_each_definition_is_child_detail_with_allowed_family(): void {
		$allowed_families = array( 'services', 'offerings', 'locations', 'products', 'directories', 'profiles', 'informational' );
		foreach ( Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions() as $def ) {
	...
	public function test_each_definition_has_hierarchy_role_leaf(): void {
		foreach ( Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions() as $def ) {
			$this->assertArrayHasKey( 'hierarchy_hints', $def );
			$role = (string) ( $def['hierarchy_hints']['hierarchy_role'] ?? '' );
			$this->assertSame( 'leaf', $role, 'Child/detail templates must have hierarchy_hints.hierarchy_role = leaf' );
		}
	}
	...
	public function test_cta_count_at_least_five_and_last_section_is_cta(): void {
		foreach ( Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertGreaterThanOrEqual( 5, $cta_count, "Child/detail variant {$internal_key} must have at least 5 CTA sections" );
	...
	public function test_preview_metadata_synthetic_only(): void {
		foreach ( Child_Detail_Variant_Expansion_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertTrue( $def['preview_metadata']['synthetic'], 'Child/detail variant templates must use synthetic preview only.' );
		}
	}
	...
	public function test_batch_id_constant(): void {
		$this->assertSame( 'PT-13', Child_Detail_Variant_Expansion_Page_Template_Definitions::BATCH_ID );
	}
}

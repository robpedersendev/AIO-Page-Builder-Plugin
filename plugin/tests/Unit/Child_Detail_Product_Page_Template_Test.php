<?php
/**
 * Unit tests for product/catalog child/detail page template batch (Prompt 161): child_detail tagging,
 * CTA counts (≥5), sequencing compliance, preview realism, one-pager presence, family metadata, export (spec §13, §14.3, §16, §17.7).
 *
...
use AIOPageBuilder\Domain\Registries\PageTemplate\ChildDetailProductBatch\Child_Detail_Product_Page_Template_Definitions;
...
require_once $plugin_root . '/src/Domain/Registries/PageTemplate/ChildDetailProductBatch/Child_Detail_Product_Page_Template_Definitions.php';
...
final class Child_Detail_Product_Page_Template_Test extends TestCase {
	...
	public function test_batch_has_expected_number_of_templates(): void {
		$defs = Child_Detail_Product_Page_Template_Definitions::all_definitions();
		$keys = Child_Detail_Product_Page_Template_Definitions::template_keys();
		$this->assertCount( count( $keys ), $defs );
		$this->assertGreaterThanOrEqual( 10, count( $defs ), 'Product/catalog child/detail batch must be substantial' );
	}

	public function test_each_definition_has_required_schema_fields(): void {
		$required = Page_Template_Schema::get_required_fields();
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
	...
	public function test_each_definition_has_child_detail_category_class_and_products_family(): void {
		$allowed_families = Child_Detail_Product_Page_Template_Definitions::ALLOWED_FAMILIES;
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertContains( $family, $allowed_families, "template_family must be products, got: {$family}" );
		}
	}

	public function test_each_definition_archetype_is_offer_or_comparison(): void {
		$allowed_archetypes = array( 'offer_page', 'comparison_page' );
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			$arch = (string) ( $def[ Page_Template_Schema::FIELD_ARCHETYPE ] ?? '' );
			$this->assertContains( $arch, $allowed_archetypes, "Product child_detail archetype must be offer_page or comparison_page, got: {$arch}" );
		}
	}
	...
	public function test_each_definition_has_parent_family_compatibility(): void {
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertContains( 'products', $compat );
		}
	}
	...
	public function test_child_detail_product_cta_count_at_least_five_and_last_section_is_cta(): void {
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertGreaterThanOrEqual( 5, count( $cta_keys ), "Product child/detail template {$internal_key} must have at least 5 CTA sections" );
	...
	public function test_preview_metadata_synthetic_only(): void {
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertTrue( $def['preview_metadata']['synthetic'], 'Product child/detail templates must use synthetic preview data only.' );
		}
	}
	...
	public function test_one_pager_includes_cta_direction_or_flow(): void {
		foreach ( Child_Detail_Product_Page_Template_Definitions::all_definitions() as $def ) {
			...
			$this->assertTrue( $has_flow || $has_cta, 'Product child/detail one-pager should include page_flow_explanation or cta_direction_summary' );
		}
	}
	...
	public function test_batch_id_constant(): void {
		$this->assertSame( 'PT-08', Child_Detail_Product_Page_Template_Definitions::BATCH_ID );
	}
}

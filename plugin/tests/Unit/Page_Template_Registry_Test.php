...
final class Page_Template_Registry_Test extends TestCase {
	...
	public function test_valid_template_creation(): void {
		...
		$GLOBALS['_aio_post_meta']['9001']     = array(
			...
			'_aio_status'             => 'active',
			'_aio_section_definition' => wp_json_encode( array_merge( $this->valid_minimal_section(), array( 'internal_key' => 'st01_hero' ) ) ),
		);
		$GLOBALS['_aio_post_meta']['9002']     = array(
			'_aio_internal_key'       => 'st05_cta',
			'_aio_status'             => 'active',
			'_aio_section_definition' => wp_json_encode(
				array_merge(
					$this->valid_minimal_section(),
					array(
						'internal_key' => 'st05_cta',
						'name'         => 'CTA',
					)
				)
			),
	...
	public function test_nonexistent_section_reference_rejection(): void {
		...
		$input[ Page_Template_Schema::FIELD_ORDERED_SECTIONS ][]                       = array(
			'section_key' => 'st99_nonexistent',
			'position'    => 2,
			'required'    => false,
		);

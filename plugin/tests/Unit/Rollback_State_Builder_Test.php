...
final class Rollback_State_Builder_Test extends TestCase {
	...
	public function test_list_rollback_entries_for_plan_returns_linked_entries(): void {
		...
		$entries = $repo->list_rollback_entries_for_plan( $plan_id );

		$this->assertCount( 1, $entries );
		$this->assertSame( $post_id, $entries[0]['post_snapshot_id'] );
		$this->assertSame( $pre_id, $entries[0]['pre_snapshot_id'] );
		$this->assertSame( 'replace_page', $entries[0]['action_type'] );
		$this->assertSame( '42', $entries[0]['target_ref'] );
		$this->assertSame( '2025-03-12T10:01:00+00:00', $entries[0]['created_at'] );
	}
}

...
- **Health report errors/warnings:** Use **Industry Health Report** (AIO Page Builder menu). Errors = broken refs or invalid profile (e.g. primary pack not found, selected bundle missing). Warnings = advisory (e.g. bundle industry mismatch). No auto-fix; guide user to fix profile or enable pack. For a structured path from detection to correction (missing refs, deprecated refs, inactive assets, stale overrides), see [industry-guided-repair-workflow-contract.md](../contracts/industry-guided-repair-workflow-contract.md).

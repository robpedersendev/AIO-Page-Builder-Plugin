...
**Extension boundaries and roadmap:** Long-term extension seams, deprecation policy, and roadmap categories are defined in [industry-subsystem-roadmap-contract.md](../contracts/industry-subsystem-roadmap-contract.md). New work must use approved seams only. **Stale-content visibility:** To prioritize which overlays, rules, docs, or bundles may need refresh (age, review recency, usage criticality, change-sensitivity), use the internal [industry-asset-aging-scoring-contract.md](../contracts/industry-asset-aging-scoring-contract.md) (Prompt 555). The report is generated by `Industry_Asset_Aging_Report_Service` and is available from the Industry Author Dashboard via **Stale content report** (Prompt 556). Stale score is advisory only; no auto-edit. **Phase-two backlog:** Completed clusters and remaining work are summarized in [industry-phase-two-backlog-map.md](industry-phase-two-backlog-map.md) for dependency-aware prioritization. **New industry candidates** must be evaluated with [future-industry-candidate-evaluation-framework.md](future-industry-candidate-evaluation-framework.md) and [future-industry-scorecard-template.md](future-industry-scorecard-template.md) before implementation. **Support and operators:** For shared terminology and troubleshooting, use [industry-support-training-packet.md](industry-support-training-packet.md) and [industry-operator-curriculum.md](industry-operator-curriculum.md). **Author dashboard:** For a single-place view of health, completeness, gaps, and release readiness, use the internal [Industry Author Dashboard](../contracts/industry-author-dashboard-contract.md) (contract and screen per Prompts 521–522). **Pack family comparison:** To compare launch industries, subtype families, and candidates side by side (completeness, coverage, caution, bundle, preset, maturity), use the internal [industry-pack-family-comparison-contract.md](../contracts/industry-pack-family-comparison-contract.md) (Prompt 557) and the comparison screen (Prompt 558). **Drift detection:** To flag when contracts, schemas, docs, or seeded asset conventions diverge, use the internal [industry-subsystem-drift-detection-contract.md](../contracts/industry-subsystem-drift-detection-contract.md) (Prompt 561) and drift report generator (Prompt 562). Findings are advisory; resolve or waive per release/maintenance policy. **Task queue:** To turn completeness, gap prioritization, and override conflicts into a single maintenance queue, use [industry-author-task-queue-contract.md](../contracts/industry-author-task-queue-contract.md) and `Industry_Author_Task_Queue_Service::generate_queue()`.

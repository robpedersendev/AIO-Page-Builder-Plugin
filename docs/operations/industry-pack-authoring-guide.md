...
| **One-pager overlays** (optional) | Page one-pager overlays keyed by industry + page_template_key; schema-valid. | industry-page-onepager-overlay-schema; industry-overlay-catalog |
...
7. **One-pager overlays** (if used) – Add page one-pager overlay definitions; load via Industry_Page_OnePager_Overlay_Registry.

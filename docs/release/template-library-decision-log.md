...
**How to append entries:** Use `Template_Deprecation_Service::build_decision_log_entry()` to generate a consistent payload; paste or sync the result into the Entries section below. See [template-ecosystem-maintenance-runbook.md](../operations/template-ecosystem-maintenance-runbook.md) §5.1. **Form provider–related decisions** (adapter changes, supported provider version, registry registration, compatibility) may use the same format with a Decision ID prefix (e.g. DL-FP-001); see [form-provider-maintenance-sop.md](../operations/form-provider-maintenance-sop.md) and [form-provider-upgrade-and-support-runbook.md](../operations/form-provider-upgrade-and-support-runbook.md).

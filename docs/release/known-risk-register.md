...
| TF-1 | Test suite | **Superseded (2026-03-21).** Prior RC1 note: 25 PHPUnit failures (2026-03-19). At git `ca94de0`, `vendor/bin/phpunit -c phpunit.xml.dist` reports **exit 0**; **3,056** tests; **5** skipped; **8** deprecations. Do not cite TF-1 as an active failure waiver for current HEAD. | N/A — closure doc §4 retracts TF-1. Track skips/deprecations in [release-candidate-closure.md](../qa/release-candidate-closure.md) §2. |
| TOOL-1 | PHPCS (`phpcs.xml.dist`) | Scoped `plugin/src` run (2026-03-21): **exit 2**; **9** errors, **11** warnings, **12** files per `--report=summary`. `plugin/tests` tree: **exit 0** (476 files). | Fix or PHPCBF; or formal waiver with owner per hardening matrix. See [RELEASE_CHECKLIST.md](../qa/RELEASE_CHECKLIST.md). |
| TOOL-2 | Plugin Check | Summarize on `tools/plugin-check/output/plugin-check-report.json` (2026-03-21): **253** ERROR, **690** WARNING, **194** files with findings (`composer run plugin-check:summarize`). | Triage, fix, or document waivers; treat as open quality debt — not a clean gate. |
| TOOL-3 | PHPStan | `composer run phpstan` (2026-03-21): **exit 1** — parallel worker hit **512M** memory limit; analysis **incomplete**. | Raise memory in script/CLI; re-run for full baseline before claiming static-analysis green. |

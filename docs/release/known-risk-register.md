...
| BP-PERSIST-1 | Build Plan / persistence | Third-party code (mu-plugins, `update_post_metadata` filters, DB triggers) can alter or truncate `_aio_plan_definition` after a successful-looking API write. The plugin detects common cases via post-write length verification, falls back to a scoped `$wpdb` insert for `aio_build_plan` only, logs `AIO_BP_PLAN_DEFINITION_PERSIST_VERIFY_FAIL`, and may still fail if infrastructure corrupts meta regardless. | Triage site-specific filters; document one-off waivers here if accepted per §5. Operators can rebuild from the source AI run when lineage allows. |

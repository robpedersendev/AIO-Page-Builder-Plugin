...
| ACF-1 | ACF registration / cache | Section-key cache is optional and TTL-bound; stale cache falls back to assignment-map resolution. Template/composition-keyed cache invalidated only by TTL unless hooks added where definitions are saved. | Correctness over speed; cache invalidates on assignment change. [acf-registration-performance-release-gate.md](acf-registration-performance-release-gate.md); [acf-conditional-registration-acceptance-report.md](../qa/acf-conditional-registration-acceptance-report.md). |

...
| TF-1 | Test suite / pre-existing failures | 25 PHPUnit failures pre-existing at RC1 (2026-03-19). Root causes: (a) 3 ACF availability tests - global mock state leak across test suite; (b) 3 Build Plan analytics tests - missing total_rollbacks key in analytics struct; (c) 4 Build Plan UI component tests - row action resolver contract drift; (d) 10 Industry subsystem tests - schema/validation/simulation contract drift; (e) 5 Crawl/Onboarding/Snapshot/Queue/Template assertion drift. None are security, data-loss, or critical functional issues. | Waiver TF-1: pre-existing; no user-facing behavior broken. Targeted fix pass planned for v1.1. See release-candidate-closure.md sec.4. |

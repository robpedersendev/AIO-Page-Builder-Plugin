...
- **Inputs**: Normalized Industry Profile (primary_industry_key, selected_starter_bundle_key, question_pack_answers as desired); optional bundle key override; optional `available_page_template_keys`, `available_section_keys`, `content_hints` (e.g. has_staff_bios, has_gallery, has_trust_proof, has_service_area_detail, has_emergency_details, has_valuation_assets). When content_hints or availability are omitted, detector may still emit **expected** gaps for the industry based on recommended structure.

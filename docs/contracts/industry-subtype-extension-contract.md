...
- **Registry**: Subtype definitions are loaded via Industry_Subtype_Registry keyed by subtype_key and by parent_industry_key. Invalid or duplicate keys are skipped at load per schema validation. Industry_Subtype_Validator validates single definitions (schema and optional parent-industry reference check); use for authoring and tests. **Subtype section-helper overlays**: Subtype_Section_Helper_Overlay_Registry (subtype-section-helper-overlay-schema.md) keyed by (subtype_key, section_key). Composition order: base helper → industry overlay → subtype overlay. When subtype overlay is absent or invalid, result is industry overlay (or base-only). **Subtype page one-pager overlays**: Subtype_Page_OnePager_Overlay_Registry (subtype-page-onepager-overlay-schema.md) keyed by (subtype_key, page_template_key). Composition order: base one-pager → industry overlay → subtype overlay. When subtype overlay is absent or invalid, result is industry overlay (or base-only).
...
- **Subtype section-helper overlay**: subtype-section-helper-overlay-schema.md; Subtype_Section_Helper_Overlay_Registry; composition order in Industry_Helper_Doc_Composer when subtype context is used.
- **Subtype page one-pager overlay**: subtype-page-onepager-overlay-schema.md; Subtype_Page_OnePager_Overlay_Registry; composition order in Industry_Page_OnePager_Composer when subtype context is used.
- **Subtype starter bundle**: subtype-starter-bundle-contract.md; Industry_Starter_Bundle_Registry supports optional subtype_key on bundle definitions and get_for_industry(industry_key, subtype_key) with fallback to industry bundles when no subtype-scoped bundles exist.
- **Subtype caution rules**: subtype-compliance-rule-contract.md; Subtype_Compliance_Rule_Registry (load, get_for_subtype). Composition order: parent industry caution rules (Industry_Compliance_Rule_Registry) then subtype rules for (parent_industry_key, subtype_key). Fallback: when no or invalid subtype, parent rules only.
- **Second-wave planning**: [launch-subtype-second-wave-planning-framework.md](../operations/launch-subtype-second-wave-planning-framework.md) (Prompt 479) — admission criteria, prioritization, and when not to add a subtype.

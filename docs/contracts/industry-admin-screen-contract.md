...
- **Industry_Section_Preview_View_Model**: DTO for the section detail screen: has_industry, recommendation_fit, composed_helper (allowed regions), substitute_suggestions, warning_flags, explanation_reasons, compliance_warnings (advisory; Prompt 407), subtype_influence (Prompt 441).
- **Section template detail screen**: When the industry section preview resolver is available (container key `industry_section_preview_resolver`), the screen merges `industry_preview` (view model to_array()) into state and renders an industry fit block in the metadata panel: industry label, fit badge, composed helper excerpt (tone_notes, cta_usage_notes), warnings, and substitute suggestion links. When a valid subtype is selected, a **subtype influence** block is shown: subtype label, whether a subtype helper overlay was applied for this section, and optional caution/summary notes. Substitute suggestions are populated only when the resolver is called with a full section list; otherwise the list is empty. Generic fallback: when no industry profile or resolver unavailable, no industry block is shown.
...
- **Section preview resolver**: `plugin/src/Domain/Industry/Registry/Industry_Section_Preview_Resolver.php`; view model: `plugin/src/Admin/ViewModels/Sections/Industry_Section_Preview_View_Model.php` (Prompt 384). Subtype influence: `plugin/src/Admin/ViewModels/Industry/Industry_Subtype_Preview_Influence_View_Model.php` (Prompt 441).
...
- **Subtype comparison screen (Prompt 442):** Internal comparison of parent-industry vs subtype starter bundles and recommendation highlights. Screen: `Industry_Subtype_Comparison_Screen` (slug `aio-page-builder-industry-subtype-comparison`). Data from `Industry_Subtype_Comparison_Service::get_comparison()`. Read-only; capability VIEW_LOGS. When no subtype is selected, shows parent column and prompts to select a subtype in Industry Profile.
- **Bundle comparison screen (Prompt 450):** Read-only diff screen for two or more starter bundles. Screen: `Industry_Starter_Bundle_Comparison_Screen` (slug `aio-page-builder-industry-bundle-comparison`). Query param `bundle_keys` (comma-separated). Data from `Industry_Starter_Bundle_Diff_Service::compare()`. Shows page-family emphasis, template refs, section refs, CTA/LPagery/preset refs, metadata notes. No auto-apply; selection remains in Industry Profile.
- **Pack family comparison screen (Prompt 558):** Internal comparison of launch-industry and subtype families by completeness band, coverage gaps, and blocker count. Screen: `Industry_Pack_Family_Comparison_Screen` (slug `aio-page-builder-industry-pack-family-comparison`). Data from `Industry_Pack_Completeness_Report_Service::generate_report()` and `Industry_Coverage_Gap_Analyzer::analyze()`. Read-only; capability VIEW_LOGS. See [industry-pack-family-comparison-contract.md](industry-pack-family-comparison-contract.md).
- **Style layer comparison screen (Prompt 549):** Read-only comparison of parent (industry) preset, goal overlay, and combined style outcome. Screen: `Industry_Style_Layer_Comparison_Screen` (slug `aio-page-builder-industry-style-layer-comparison`). Query param `preset_key` (optional). Data from `Industry_Style_Layer_Diff_Service::compare()`. Shows token and component-override differences; fallback when layers missing. No style apply; capability VIEW_LOGS.
- **Conversion goal comparison screen (Prompt 515):** Read-only comparison of no-goal, current-goal, and optional alternate-goal bundle/Build Plan posture. Screen: `Conversion_Goal_Comparison_Screen` (slug `aio-page-builder-industry-conversion-goal-comparison`). Query param `alternate_goal_key` (optional). Data from `Industry_What_If_Simulation_Service::run_simulation()` with `PARAM_ALTERNATE_CONVERSION_GOAL` per scenario. Shows profile summary, comparison bundles/templates/sections, and CTA/posture note per goal. No auto-apply; capability VIEW_LOGS.
- **Industry Author Dashboard (Prompt 522):** Internal summary screen for pack health, completeness, release readiness, scaffold status, and coverage gaps. Screen: `Industry_Author_Dashboard_Screen` (slug per contract). Data from Health_Check_Service, Completeness_Report_Service, Coverage_Gap_Analyzer; view model aggregates for widgets. Read-only; links to Health Report, comparison screens, release gate, maintenance checklist. See [industry-author-dashboard-contract.md](industry-author-dashboard-contract.md).

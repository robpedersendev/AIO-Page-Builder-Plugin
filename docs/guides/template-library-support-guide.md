...
| ACF local JSON or sync for plugin groups | Plugin-owned groups (group_aio_*) are registered from the plugin registry only. Do not save them to ACF’s native acf-json path or use ACF sync for them. Plugin mirror is for export/diff only. | [acf-local-json-coexistence.md](../contracts/acf-local-json-coexistence.md). |
| Inspect scoped registration for a page/template | Internal utility: resolve `scoped_registration_inspection_service` and call `inspect_for_page($page_id)` or `inspect_for_new_page_template($key)` / `inspect_for_new_page_composition($id)` to see resolved section keys and group keys without loading the editor. | [acf-scoped-registration-inspection-guide.md](../operations/acf-scoped-registration-inspection-guide.md). |

...
| ACF groups missing or wrong on page edit | Registration is conditional: only sections assigned to the page (or chosen template/composition on new page) are registered. If assignments are wrong, reassign or check assignment map. Section-key cache invalidates when assignments change. | Assignment map; [acf-conditional-registration-diagnostics-checklist.md](../qa/acf-conditional-registration-diagnostics-checklist.md) for diagnostics. |
